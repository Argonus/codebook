...
| DenseNet121_v4        | 0.1660 | 0.1029 | 0.3517 | 25 |
| DenseNet121_v4_1      | 0.1683 | 0.0819 | 0.2071 | 29 |
| DenseNet121_v4_2      | 0.1611 | 0.0783 | 0.1833 | 28 |
| DenseNet121_v5        | 0.1484 | 0.0690 | 0.1677 | 29 |
...
| DenseNet121_v4        | 0.7568 | 0.7278 | 0.8617 | 14 |
| DenseNet121_v4_1      | 0.7705 | 0.7364 | 0.8085 | 30 |
| DenseNet121_v4_2      | 0.7782 | 0.7373 | 0.8028 | 30 |
| DenseNet121_v5        | 0.7745 | 0.7354 | 0.7941 | 28 |
...
| DenseNet121_v4        | 1.0000 | 0.3434 | 0.6714 | 1  |
| DenseNet121_v4_1      | 0.5000 | 0.3390 | 0.5666 | 9  |
| DenseNet121_v4_2      | 0.4531 | 0.2993 | 0.5607 | 18 |
| DenseNet121_v5        | 0.4561 | 0.3117 | 0.5536 | 16 |
...
| DenseNet121_v4        | 0.1361 | 0.0808 | 0.2768 | 30 |
| DenseNet121_v4_1      | 0.1383 | 0.0626 | 0.1539 | 29 |
| DenseNet121_v4_2      | 0.1372 | 0.0607 | 0.1343 | 23 |
| DenseNet121_v5        | 0.1192 | 0.0518 | 0.1227 | 29 |
...
| DenseNet121_v4        | Cardiomegaly (F1=0.33) | Effusion (F1=0.28)      | Mass (F1=0.24)         |
| DenseNet121_v4_1      | Effusion (F1=0.45)     | Hernia (F1=0.33)        | Cardiomegaly (F1=0.32) |
| DenseNet121_v4_2      | Effusion (F1=0.41)     | Cardiomegaly (F1=0.39)  | Hernia (F1=0.31)       |
| DenseNet121_v5        | Effusion (F1=0.44)     | Cardiomegaly (F1=0.33)  | Hernia (F1=0.25)       |
...
| DenseNet121_v4         | 1.6632 | 0.1693 | 0.0515 | 0.2718 |
| DenseNet121_v4_1       | 1.7498 | 0.1845 | 0.0540 | 0.2866 |
| DenseNet121_v4_2	     | 1.7078	| 0.1871 | 0.0524	| 0.2773 |
| DenseNet121_v5         | 1.5949	| 0.1890 | 0.0485	| 0.2548 |
...
| DenseNet121_v3_3       | Yes | 12 | High              | 92.2% (Moderate Drop (Best Near End))    |
| DenseNet121_v4         | Yes | 12 | High              | 92.2% (Moderate Drop (Best Near End))    |
| DenseNet121_v4_1       | Yes | 21 | Medium            | 86.7% (Moderate Drop (Best Near End))    |
| DenseNet121_v4_2       | Yes | 13 | High              | 81.2% (Moderate Drop (Best Near End))    |
| DenseNet121_v5         | Yes | 8  | High              | 91.5% (Moderate Drop (Best Near End))    |
...
| DenseNet121_v4         | 440.1 | 1065.9| 1412.9 |
| DenseNet121_v4_1       | 174.8 | 925.5 | 1238.7 |
| DenseNet121_v4_2       | 265.0 | 807.2 | 1126.9 |
| DenseNet121_v5         | 222.3 | 709.2 | 988.7  |
...
| DenseNet121_v3_2	| Cardiomegaly (AUC=0.84, F1=0.33, wt=2.5) <br/> Effusion (AUC=0.80, F1=0.28, wt=0.5) <br/> Mass (AUC=0.72, F1=0.24, wt=1.2) | Pneumothorax (AUC=0.76, F1=0.04, wt=4.9) <br/> Pleural_Thickening (AUC=0.68, F1=0.00, wt=1.1) <br/> Atelectasis (AUC=0.68, F1=0.03, wt=0.6) | ? |
| DenseNet121_v4	| Cardiomegaly (AUC=0.84, F1=0.33, wt=2.5) <br/> Effusion (AUC=0.80, F1=0.28, wt=0.5) <br/> Mass (AUC=0.72, F1=0.24, wt=1.2) | Pneumothorax (AUC=0.76, F1=0.04, wt=4.9) <br/> Pleural_Thickening (AUC=0.68, F1=0.00, wt=1.1) <br/> Atelectasis (AUC=0.68, F1=0.03, wt=0.6) | ? |
| DenseNet121_v4_1	| Effusion (AUC=0.83, F1=0.45, wt=0.5) <br/> Hernia (AUC=0.84, F1=0.33, wt=30.2) <br/> Cardiomegaly (AUC=0.87, F1=0.32, wt=2.5)	| Consolidation (AUC=0.75, F1=0.00, wt=1.5) <br/> Pleural_Thickening (AUC=0.71, F1=0.01, wt=1.1) <br/> Pneumonia (AUC=0.68, F1=0.00, wt=2.0) | ? |
| DenseNet121_v4_2	| Effusion (AUC=0.85, F1=0.41, wt=0.5) <br/> Cardiomegaly (AUC=0.89, F1=0.39, wt=2.5) <br/> Hernia (AUC=0.88, F1=0.31, wt=30.2)	| Consolidation (AUC=0.77, F1=0.00, wt=1.5) <br/> Atelectasis (AUC=0.75, F1=0.03, wt=0.6) <br/> Pleural_Thickening (AUC=0.72, F1=0.03, wt=1.1) | ? |
| DenseNet121_v5	| Effusion (AUC=0.85, F1=0.44, wt=0.5) <br/> Cardiomegaly (AUC=0.88, F1=0.33, wt=2.5) <br/> Hernia (AUC=0.85, F1=0.25, wt=30.2) | Pneumothorax (AUC=0.81, F1=0.03, wt=4.9) <br/> Consolidation (AUC=0.76, F1=0.00, wt=1.5) <br/> Atelectasis (AUC=0.74, F1=0.04, wt=0.6) | ? |
...
### DenseNet121_v4
- **Changes from v3_2:**
  - Removed No Finding class from training
- **Hypothesis:**
  - Removing No Finding class from training will allow model to focus more on predicting disases
- **Impact:** [Results](models/DenseNet121_v4)
  - F1 score decreased but it's stil high and model is more focusing on diseases
  - Training stabilized vs previous model

### DenseNet121_v4_1
- **Changes from v4:**
  - Add more aggressive data augmentation
- **Hypothesis:**
  - Looking at model validation vs training scores, it starts to show signs of overfitting. More aggressive data augmentation should help.
- **Impact:** [Results](models/DenseNet121_v4)
  - Improved validation F1 score (0.1683), demonstrating better generalization 
  - Reduced gap between training and validation performance, indicating less overfitting
  - More stable learning trajectory in later epochs with an upward trend

### DenseNet121_v4_2
- **Changes from v4_1:**
  - Modified augmentation strategy with increased randomness in transformations
- **Hypothesis:**
  - A more randomized approach to augmentation might provide better regularization while preserving important diagnostic features in the X-ray images
- **Impact:** [Results](models/DenseNet121_v4_2)
  - Comparable validation F1 score (0.1611) with slightly higher AUC (0.7782)
  - Better performance on Cardiomegaly (F1=0.39, improved from 0.32 in v4_1)
  - More consistent AUC scores across difficult classes
  - Higher volatility in validation metrics during training
  - Reduced computational cost with 13% faster training time than v4_1

### DenseNet121_v5
- **Changes from v4_2:**
  - Added spatial attention
- **Hypothesis:**
  - Spatial attention might help focus on specific regions of interest in the X-ray images, potentially improving performance on specific classes
- **Impact:** [Results](models/DenseNet121_v5)
  - Lower F1 score (0.1611) compared to v4_2
  - Lower AUC score (0.7782) compared to v4_2
  - Better performance on low performance classes
...
### Gradient
Gradient is a vector of partial derivatives of a function with respect to its inputs. In this context, it is used to calculate the direction of steepest ascent of the loss function.
